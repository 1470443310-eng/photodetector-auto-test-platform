...
运行测试后，主页面和下载的 HTML 报告均展示数据集来源、生成链路、测量条件与种子；明确区分厂商量级参考和程序生成数据。新增 PASS/FAIL/ERROR 数量图，以及暗电流、响应度、暗噪声、NEP、D*、重复性、线性度、稳定性共 8 项全样本直方图和逐颗散点。

每张图标明单位、有效/排除样本数、均值、中位数、样本标准差和当前规格；ERROR、缺失与非有限值不会填零。正值跨量级指标使用 log10 坐标，非正值保留并退回线性坐标。首测良率和有效良率分别注明分母；单批统计不代表真实制造良率，也不是跨批次良率分布。

页面可下载测量点 CSV、指标 CSV、结果 JSON 和包含模式与有效种子的完整配置 JSON。图表以 SVG 嵌入 HTML，下载后无需联网即可查看。复现时在相同版本环境中读取配置 JSON 并调用 `src.sequences.test_sequence.run_batch(plan)`。

```bash
python -m pytest
```

当前完整链路为：探测器模型 → 虚拟 SMU/光源 → 自动采集 → 数据清洗 → 参数计算 → 规格判定与失败归因 → Batch 质量统计 → 异常检测 → Quality Report。网页默认使用现实波动模式，每批依据可配置的器件缺陷概率重新抽样，并叠加制造离散、温度和仪器误差，因此 20 DUT 的 PASS/FAIL/ERROR 数量会随批次合理变化；报告保存随机种子，可完整复现。教学复现模式继续使用固定种子和固定质量结构。

当前自动化回归：20 passed（包含主页面报告、右上角说明入口、连续运行随机种子变化、批次质量数量变化以及反馈迁移幂等性测试）。
