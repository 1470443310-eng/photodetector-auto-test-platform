...
本项目是面向光电信息科学与工程本科生、半导体测试开发岗位的作品 Demo：基于虚拟仪器完成光电二极管暗态/光照 I-V、响应度、重复性、三态判定、批量质量统计和自动报告，并逐步迁移到真实 VISA/SCPI 仪器。
...
1. 在用户电脑复现 8 项测试和 20 DUT 基线。
2. 安装完整依赖并验证 `streamlit run app.py`。
3. 让用户本人修改一项阈值，预测、运行并解释结果。
4. 让用户主动制造一个故障，完成定位、修复和回归记录。
5. 选择真实光电二极管、SMU 与光源型号后，依据 datasheet 和编程手册修改参数及 SCPI 命令。
6. 整理 Git 提交记录、演示视频和博士审阅包。
